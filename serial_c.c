...
int open_port(int port)
{
	int fd;
	char filename[128];
//	snprintf(filename, sizeof(filename), "/dev/ttyUSB0");
...
static int mod_detected(int mod,StateInfo *stateinfo){
	...
	case 15:
		...
		break;
	case 16:
		strcpy(stateinfo->phone,"PHONE NUMBER");
		break;
	default:
...
int get_phone(int fd,StateInfo *stateinfo)
{
	...
	while(!flag)
	{
		while((nread=read(fd,buff,SIZE))>0)
		{
			...
			{
				strcpy(stateinfo->phone,"ERROR");
			}
			else
			{
				perror("phone number detect internal error");
				return 1;
			}
		}
		//usleep(1000);        -> dead if added
